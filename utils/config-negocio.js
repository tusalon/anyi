...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '4bc5cb02-dd92-4d7f-acf9-aa240fea8f93'; // ID de Anyi
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53752030';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'angelatahimia@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Anyi!';
};
...
console.log('✅ config-negocio.js listo para Anyi');
